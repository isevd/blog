...
const FormEditArticle = ({ pageTitle, article, onSubmit }) => {
  ...
  const createTagInput = (index) => (
    <input
      ...
      onChange={(event) =>
        setTagValues((tags) =>
          tags.map((tag, idx) => (idx === index ? event.target.value : tag))
        )
      }
    />
  )
    ...
    for (let i = 0; i < number; i++) {
      const btnAddTag = (
        <button
          ...
          onClick={() => setTagValues([...tagValues, ''])}
        >
          Add tag
        </button>
      )
      const btnDeleteTag = (
        <button
          ...
          onClick={() =>
            setTagValues([...tagValues.slice(0, i), ...tagValues.slice(i + 1)])
          }
        >
          Delete
        </button>
  ...
  return (
    <div className={style['new-article-container']}>
      ...
      <form
        ...
        onSubmit={handleSubmit((data) => onSubmit(data, tagValues))}
      >
        <label htmlFor="title">
          <span className={style['text-label']}>Title</span>
          <input
            className={classNames(style['text-input'], {
              [style['error-input']]: errors.title,
            })}
            type="text"
            id="title"
            placeholder="Title"
        ...
        <label htmlFor="description">
          ...
          <input
            className={classNames(style['text-input'], {
              [style['error-input']]: errors.description,
            })}
            type="text"
            id="description"
        ...
        <button
          className={classNames(
            style.btn,
            !isValid && style['btn-disable'],
            isSubmitted && style['btn-disable']
          )}
          type="submit"
